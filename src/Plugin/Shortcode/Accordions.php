 ...
 *   description = @Translation("Insert a Accordions.")
 * )
 */
class Accordions extends ShortcodeBaseEx {

	public function getThemeVars($with_hash = false, $merge = []) {
		//accordions is autoresolvable by ++, not for user input
		$res = parent::getThemeVars($with_hash, array_merge(['accordions'=>null], $merge));
		return $res;
	}
